 ...
 * 
 * Run: node tests/phase1-production-hub-test.js
 */
...
runTest('SceneBuilderPanel should not import useEditor', () => {
...
runTest('SceneBuilderPanel should use screenplay context', () => {
  const filePath = path.join(process.cwd(), 'components/production/SceneBuilderPanel.tsx');
  const content = fs.readFileSync(filePath, 'utf-8');
...
runTest('ProductionHub should use getToken for Jobs Banner', () => {
...
runTest('AssetBankPanel should use getToken for API calls', () => {
...
runTest('MediaLibrary should use S3 pre-signed URLs', () => {
...
runTest('MediaLibrary upload flow is correct', () => {
  const filePath = path.join(process.cwd(), 'components/production/MediaLibrary.tsx');
  const content = fs.readFileSync(filePath, 'utf-8');
...
runTest('Backend media.ts route file exists', () => {
...
runTest('Media routes are registered in server.ts', () => {
...
runTest('Media routes have correct implementation', () => {
  const filePath = path.join(process.cwd(), '..', 'website-backend-api', 'src', 'routes', 'media.ts');
  const content = fs.readFileSync(filePath, 'utf-8');
...
runTest('No conflicting imports in fixed files', () => {
  const files = [
...
runTest('All auth calls use Clerk getToken pattern', () => {
  const files = [
    'components/production/ProductionHub.tsx',
    'components/production/AssetBankPanel.tsx',
    'components/production/MediaLibrary.tsx',
