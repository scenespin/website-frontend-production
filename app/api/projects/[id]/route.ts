...
/**
 * PUT /api/projects/[id]
 * Proxy to backend to update a project
 */
export async function PUT(
  ...
  try {
    console.log('[Project Update API] Request received');
    const { userId, getToken } = await auth();
    
    if (!userId) {
      console.error('[Project Update API] No userId found');
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
    ...
    console.log('[Project Update API] User authenticated:', userId);
    // Get token with wryda-backend template for backend API
    const token = await getToken({ template: 'wryda-backend' });
    if (!token) {
      console.error('[Project Update API] Could not generate token');
      return NextResponse.json(
        { error: 'Unauthorized - Could not generate token' },
        { status: 401 }
      );
    ...
    console.log('[Project Update API] Updating project:', id, 'with updates:', Object.keys(body));
    ...
    if (!backendResponse.ok) {
      ...
      console.error('[Project Update API] Backend error:', backendResponse.status, errorData);
      return NextResponse.json(errorData, { status: backendResponse.status });
    }

    const data = await backendResponse.json();
    console.log('[Project Update API] Successfully updated project');
    return NextResponse.json(data);
  } catch (error: any) {
