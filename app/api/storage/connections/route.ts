 ...
 * Proxy to backend to get all connected storage providers for user
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Proxy to backend
    const backendResponse = await fetch(`${BACKEND_API_URL}/api/storage/connections`, {
      method: 'GET',
      headers: {
    ...
    console.error('[Storage Connections API] Error:', error);
    return NextResponse.json(
      { 
