...
export async function GET(
  ...
  try {
    ...
    const operationType = request.nextUrl.searchParams.get('operationType') || 'video-generation';
    const limit = request.nextUrl.searchParams.get('limit') || '50';

    const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'https://api.wryda.ai';
    const url = `${backendUrl}/api/admin/pricing/registry/${encodeURIComponent(providerId)}?operationType=${encodeURIComponent(operationType)}&limit=${encodeURIComponent(limit)}`;
    ...
    console.error('[Admin Pricing Registry Provider Proxy] Error:', error);
