...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      // Only redirect if not already on auth pages and not in development
      if (typeof window !== 'undefined' && 
          !window.location.pathname.includes('/sign-in') && 
          !window.location.pathname.includes('/sign-up')) {
        console.warn('[API] 401 Unauthorized - API might be down or token invalid');
        // Don't auto-redirect in dev, just log the error
        if (process.env.NODE_ENV === 'production') {
          // Only redirect in production after a delay to prevent loops
          setTimeout(() => {
