// API Client for Wryda.ai Backend
...
export const api = {
  ...
  cloudStorage: {
    listFiles: (provider) => apiClient.get(`/api/storage/list/${provider}`),
    downloadFile: (provider, fileId) => apiClient.get(`/api/storage/download/${provider}/${fileId}`),
    uploadFile: (provider, data) => apiClient.post(`/api/storage/upload`, { provider, ...data }),
  },
