  ...
  async (config) => {
    ...
    if (typeof window !== 'undefined' && getTokenFunction) {
      try {
        ...
        console.log('[API Interceptor] Token received:', token ? `${token.substring(0, 20)}...` : 'NULL');
        if (token) {
          config.headers.Authorization = `Bearer ${token}`;
          console.log('[API Interceptor] ✅ Authorization header set');
        } else {
          console.error('[API Interceptor] ❌ Token is null/undefined');
        }
      } catch (error) {
        console.error('[API Interceptor] Error getting auth token:', error);
      }
    } else {
      console.warn('[API Interceptor] ⚠️ Not running or getTokenFunction not set:', {
        isWindow: typeof window !== 'undefined',
        hasGetter: !!getTokenFunction
      });
    }
    return config;
  },
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      // Log the error but DON'T redirect automatically to prevent loops
      // The user will see the error and can manually go to sign-in
      console.warn('[API] 401 Unauthorized - Please sign in');
      console.warn('[API] Request:', error.config?.url);
    }
    return Promise.reject(error);
  }
);
