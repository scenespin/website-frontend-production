 ...
 * 
 * Displays a warning banner when the editor is locked by another device,
...
export default function EditorLockBanner({
  ...
  // Warning banner for locked editor (same user, different device)
  if (isLocked) {
    return (
      <div className="bg-yellow-500/20 border-b border-yellow-500/30 px-4 py-2 flex items-center gap-2 text-yellow-200">
        ...
        <span className="text-sm font-medium">
          Editor is open on another device. Read-only mode.
        </span>
      </div>
    );
  }
