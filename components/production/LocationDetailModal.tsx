...
export function LocationDetailModal({
  ...
  const allImages = useMemo(() => {
    ...
    angleVariations.forEach((variation: any) => {
      ...
      const getImageUrl = () => {
        // 🔥 FIX: Gen-4 images (and other non-square images) are already 16:9, no cropping needed
        // They don't have originalS3Key or cropped metadata - both views should show the same image
        const isGen4Image = variation.metadata?.providerId === 'runway-gen4-image' || 
                           variation.metadata?.providerId === 'runway-gen-4' ||
                           !variation.metadata?.originalS3Key; // No originalS3Key means it's not a square image
        
        if (isGen4Image) {
          // Gen-4 images are already 16:9 - both "Square" and "Cropped" views show the same image
          return variation.imageUrl || '';
        }
        
        if (viewMode === 'original') {
          // Show original square version (only for Nano Banana Pro and other square images)
          // Use fresh presigned URL from metadata (generated by backend from originalS3Key)
          if (variation.metadata?.originalImageUrl) {
            return variation.metadata.originalImageUrl; // Fresh URL from backend
          }
          // Fallback: If no metadata indicates cropping, variation.imageUrl might be the original
          // This handles older images generated before we added auto-cropping
          if (!variation.metadata?.autoCropped && !variation.metadata?.userCropped) {
            return variation.imageUrl || ''; // Might be original square for older images
          }
          return ''; // No original available
        } else {
          // Show cropped 16:9 version (only for Nano Banana Pro and other square images)
          ...
          // Primary display: variation.imageUrl is cropped 16:9 for new images, might be original for older images
          // If metadata indicates it was cropped, use it; otherwise it might be original (fallback)
          if (variation.metadata?.autoCropped || variation.metadata?.userCropped) {
            return variation.imageUrl || ''; // Primary display (already cropped 16:9)
          }
          // For older images without metadata, assume variation.imageUrl is what we want to show
          return variation.imageUrl || '';
        }
      };
