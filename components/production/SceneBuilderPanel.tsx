...
/** Shot-ordered list entry (used internally for completion handling) */
export type ShotOrderedEntry = { shotSlot: number; firstFrameUrl?: string; videoUrl?: string; videoS3Key?: string };

/** Generation history item (used internally for completion handling) */
interface GenerationHistoryItem {
  ...
  totalCredits: number;
  /** Shot-ordered list (first frame per shot; videoUrl when dialogue video opted-in) */
...
function SceneBuilderPanelInternal({ projectId, onVideoGenerated, isMobile = false, simplified = false }: SceneBuilderPanelProps) {
  ...
  // Note: loadHistory/saveHistory removed - Shot Board uses Media Library directly
  ...
  async function handleGenerate() {
    ...
    if (sceneAnalysisResult?.shotBreakdown) {
      ...
      if (dialogueChar) shotCharacterIds.add(dialogueChar.id);
        ...
        // For extreme close-up workflows, allow any user-selected character reference.
        // Framing is driven by workflow prompt modifiers rather than a required poseId.
      }
  ...
  async function handleDialogueGeneration(dialogueInfo: { hasDialogue: boolean; characterName?: string; dialogue?: string }) {
    ...
    try {
      ...
      if (data.success && data.result) {
        ...
        // Note: History removed - Shot Board uses Media Library directly
  ...
  async function handleWorkflowGeneration() {
    ...
    try {
      ...
      // 🔥 Backend handles first frame generation automatically
      // Scene builder just sends data (references, shot breakdown, scene context) to backend
  ...
  // Note: handleSendToTimeline removed - Timeline integration moved to Shot Board
  ...
  function handleGenerationComplete(execution: WorkflowStatus) {
    ...
    // Build shot-ordered list (first frame per shot + video when opted-in)
    ...
    // Retry mechanism ensures videos appear in Shot Board even if registration is still in progress
    if (projectId) {
  ...
  return (
    <div className="h-full overflow-auto bg-[#0A0A0A]">
      <div className="p-3 md:p-4 space-y-3 pb-20">
            ...
            {currentStep === 2 && wizardStep === 'shot-config' && selectedSceneId && sceneAnalysisResult && (() => {
              ...
              return (
                <ShotConfigurationStep
                  ...
                  projectId={projectId}
                  explicitCharacters={[...new Set(explicitCharacters)]}
        ...
        {/* Note: Generation history / Shot Board moved to Shot Board tab */}
        </div>
      </div>
