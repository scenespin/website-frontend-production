 ...
 *
 * Dialogue: shows Camera Angle, Video Model, Shot Duration when video opt-in and not lip-sync.
 * Action/Establishing: LEGACY — video model dropdown hidden. Action-shot video uses Elements path
 * (refs + prompt → VEO); no provider picker. A future "Elements provider" section can be added here.
...
export function VideoGenerationSelector({
  ...
  // Legacy: for action/establishing we no longer show the video model dropdown (Runway/Luma/VEO).
  // Action-shot video uses the Elements path (VEO by default). A provider section for Elements can be added later.
  if (shotType === 'action' || shotType === 'establishing') {
    return null;
  }
