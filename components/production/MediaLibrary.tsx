...
export default function MediaLibrary({
  ...
  const [showFolderSidebar] = useState(true);
              ...
              {displayLoading || folderTreeLoading ? (
                ...
                >
                  ...
                  {sortedFiles.map((file) => {
                    ...
                    >
                      {/* Selected checkmark (multi-select picker mode). */}
                      {allowMultiSelect && selectedFiles.has(file.id) && (
                      ...
                      <div className={`${viewMode === 'grid' ? 'mb-2 md:mb-3' : ''} flex-shrink-0 relative`}>
                        {(() => {
                          ...
                          return file.fileType === 'image' && fileUrl ? (
                            ...
                            />
                          ) : file.fileType === 'video' && (fileUrl || (file.thumbnailS3Key && bulkPresignedUrls?.get(file.thumbnailS3Key))) ? (
                            <SharedVideoThumbnail 
                              videoUrl={getFileUrl(file, false) || fileUrl || ''} 
                              thumbnailUrl={file.thumbnailS3Key && bulkPresignedUrls ? (bulkPresignedUrls as Map<string, string>).get(file.thumbnailS3Key) : undefined}
                              fileName={file.fileName}
