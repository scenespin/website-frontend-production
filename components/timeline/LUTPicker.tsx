...
export function LUTPicker({ asset, onSelect, onClose }: LUTPickerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-base-200 border border-base-content/20 rounded-xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-base-content/20 bg-gradient-to-r from-orange-900/20 to-base-300">
          <div>
            ...
            <p className="text-base-content/60 text-sm mt-1">
              Professional Hollywood looks • 34 presets included
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 bg-base-300/30 border-b border-base-content/20">
          <div className="relative rounded-lg overflow-hidden bg-base-300 aspect-video max-h-64">
            {/* Preview Placeholder */}
            ...
            <div className="absolute top-4 left-4 right-4 flex justify-between pointer-events-none">
              <Badge className="bg-black/70 text-base-content">Before</Badge>
              <Badge className="bg-orange-500/90 text-base-content">After</Badge>
            </div>
          </div>
        </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${
                selectedCategory === cat
                  ? 'bg-orange-600 text-base-content shadow-lg shadow-orange-500/30'
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredLUTs.map(lut => (
              <button
                ...
                className={`group relative bg-base-300 hover:bg-base-content/10 border-2 rounded-lg p-4 transition-all hover:shadow-xl hover:scale-105 ${
                  selectedLUT === lut.id
                    ? 'border-orange-500 shadow-lg shadow-orange-500/30'
                    : 'border-base-content/20 hover:border-orange-500'
                }`}
              >
                {/* Preview Thumbnail */}
                <div className="aspect-video bg-gradient-to-br from-base-content/20 to-base-300 rounded mb-2 flex items-center justify-center">
                  {lut.isDefault ? (
                    <span className="text-3xl">✨</span>
                  ) : (
                    <Palette className="w-8 h-8 text-base-content/30" />
                  )}
                </div>
                
                {/* Name */}
                <div className="text-base-content text-sm font-medium mb-1 truncate">
                  {lut.name}
                </div>
        ...
        <div className="p-6 border-t border-base-content/20 bg-base-300/50">
          ...
          <div className="mb-6">
            <label className="block text-base-content text-sm font-medium mb-2">
              Color Grade Intensity: {Math.round(intensity * 100)}%
            </label>
            <input
              type="range"
              min="0"
              ...
              className="w-full h-2 bg-base-content/20 rounded-lg appearance-none cursor-pointer accent-orange-600"
            />
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Button
              onClick={onClose}
              className="bg-orange-600 hover:bg-orange-700 text-base-content"
            >
              Done
            </Button>
          </div>
